<!-- Generated from https://kotlin-toolchain.org/0.12/user-guide/plugins/topics/checks/ (docs 0.12) on 2026-08-26. Do not edit by hand; run sync_docs.py to refresh. -->
...
To run checks, one can use the `check` command. If no arguments are provided, the command will launch all the tests (similar to the `test` command) and all the checks registered in the plugins.
...
To create a check, you need to [write a task](../tasks/) and register it as a check.
