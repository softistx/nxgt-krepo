...
The Kotlin Toolchain was built from the start with [Kotlin Multiplatform](https://kotlinlang.org/docs/multiplatform/kmp-overview.html) in mind. Kotlin Multiplatform is a technology that allows building a single module for different target platforms.
...
In multiplatform modules, some source directories and sections in the configuration files can be platform-specific. The Kotlin Toolchain defines a special suffix, called the `@platform` qualifier, to mark such platform-specific things.
...
Here is an overview of what the layout of a multiplatform module looks like when `jvm`, `iosArm64`, and `iosSimulatorArm64` platforms are enabled:
...
Example: To add the [KmLogging library](https://github.com/DiamondEdge1/KmLogging) to a multiplatform module, simply write
...
- Consuming: Kotlin code can use APIs from existing platform libraries, e.g. jars on JVM (later CocoaPods on iOS too). For Kotlin/Native, you can use raw C and Objective-C libraries via the [cinterop](../advanced/native-interop/) integration.
