<!-- Generated from https://kotlinlang.org/docs/multiplatform/compose-test.html (v) on 2026-08-28. Do not edit; re-run fetch_docs.py. -->

# Testing Compose Multiplatform UI

UI testing in Compose Multiplatform is implemented using the same finders, assertions, actions, and matchers as the Jetpack Compose testing API. If you're not familiar with them, read the [Jetpack Compose guide](https://developer.android.com/jetpack/compose/testing) before you continue with this article.

The API is [Experimental](supported-platforms.html#compose-multiplatform-ui-framework-stability-levels). It may change in the future.
...
However, JUnit-based API is available for [desktop targets](compose-desktop-ui-testing.html).
...
To provide concrete examples, the instructions on this page follow the project structure generated by the [Kotlin Multiplatform wizard](https://kmp.jetbrains.com/). If you are adding tests to an existing project, you may have to replace `shared` in paths and commands with the module name you are testing.
...
- For a general overview of testing in a Kotlin Multiplatform project, see [Understand basic project structure](multiplatform-discover-project.html#integration-with-tests) and the [Test your multiplatform app](multiplatform-run-tests.html) tutorial.
- For details on setting up and running JUnit-based tests for desktop targets, see [Testing Compose Multiplatform UI with JUnit](compose-desktop-ui-testing.html).
- For localization tests, see [Testing locales on different platforms](compose-localization-tests.html#testing-locales-on-different-platforms).
- More advanced testing in Android Studio, including automation, is covered in the [Test your app](https://developer.android.com/studio/test) article in the Android Studio documentation.
