<!-- Generated from https://kotlinlang.org/docs/multiplatform/whats-new-compose-112.html (v) on 2026-08-28. Do not edit; re-run fetch_docs.py. -->
...
- Window and dialog API v2 for desktop

You can find the full list of changes for this release on [GitHub](https://github.com/JetBrains/compose-multiplatform/releases/tag/v1.12.0). For details about specific component versions, refer to the Dependencies section.
...
The previous version, used in Compose Multiplatform 1.11, was Milestone 144. You can see the changes made between these versions in the [release notes](https://skia.googlesource.com/skia/+/refs/heads/chrome/m150/RELEASE_NOTES.md).
...
Compose Hot Reload now ships with an experimental [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) server that lets AI coding agents interact directly with your running Compose application.
...
For the full list of MCP tools available to the AI agent and how to connect it, see [MCP server for AI agents](compose-hot-reload.html#mcp-server-for-ai-agents).

### Window and dialog API v2

We've introduced a new experimental v2 API for `WindowState` and `DialogState` on desktop, addressing several limitations of the existing API. The v2 API is available in the `androidx.compose.ui.window.v2` subpackage.
...
The v2 API also unlocks scenarios that weren't possible before, such as sizing a window to its content's size while still letting the content expand (via modifiers like `fillMaxSize()`) when the window is larger. See the [Window and dialog API v2](compose-desktop-top-level-windows-management.html#window-and-dialog-api-v2) documentation page for details.
