<!-- Generated from https://kotlinlang.org/docs/multiplatform/compose-popups.html (v) on 2026-08-28. Do not edit; re-run fetch_docs.py. -->
...
Use [`Dialog()`](https://developer.android.com/reference/kotlin/androidx/compose/ui/window/Dialog.composable) when you need to interrupt the user and require a decision before they continue, for example, react to confirmations, alerts, or short forms. For separate OS-level dialogs on desktop, see [`DialogWindow()`](compose-desktop-top-level-windows-management.html#show-dialogs). Use `Popup()` for lightweight, non-blocking overlays that stay anchored to a component inside the current window, such as dropdowns, tooltips, and menus.
...
- `dismissOnBackPress` dismisses the popup on Android's back button or the Esc key on desktop, enabled by default. Requires `focusable = true`.
