...
- **Every PR targets `develop`, never `main`.** Branch off `develop`, `gh pr create --base develop`,
  ...
  opening a PR; `gh pr create` defaults to the repo's default branch, which is `main` here, so the
  flag is not optional. AGENTS.md's *Conventions* section has the repair if `main` has been written
  to directly.
