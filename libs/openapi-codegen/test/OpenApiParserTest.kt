...
class OpenApiParserTest : FunSpec({
    ...
    test("groups operations by tag, one interface per tag") {
        ...
        model.groups.first { it.name == "CategoriesApi" }.operations.map { it.name } shouldContainExactly
            listOf("deleteCategory", "findCategories")
    }

    test("maps method, path and query parameters") {
        val op = model.groups.first { it.name == "CategoriesApi" }.operations.first { it.name == "findCategories" }
    ...
    test("path parameters are always required") {
        val op = model.groups.first { it.name == "CategoriesApi" }.operations.first { it.name == "deleteCategory" }
    ...
    test("json request bodies become a single body parameter") {
        val op = model.groups.first { it.name == "TagsApi" }.operations.single()
