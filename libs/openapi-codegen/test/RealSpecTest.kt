...
class RealSpecTest : FunSpec({

    val spec = generateSequence(Path.of(System.getProperty("user.dir")).toAbsolutePath()) { it.parent }
    ...
    test("parses the demo spec end to end") {
        ...
        model.groups.map { it.name } shouldContainAll listOf(
            "AuthApi", "CategoriesApi", "RolesApi", "TagsApi", "UploadsApi", "UsersApi",
        )
