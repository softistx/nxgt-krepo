...
## Why this is not a package in shared-ktor
...
plugins outside the libraries they wrap. The library knows the backend, `shared-ktor` knows the
...
Every backend is declared `compile-only`, the way `shared-ktor` declares its own, and for the same
reason: an application that calls `redisModule` already depends on `shared-redis` — `RedisConfig` is
