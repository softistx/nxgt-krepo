...
The Ktor plugin for `stx-graphql`. One engine per application, `POST` and `GET` at `/graphql`.
...
`injectable = true` registers that same engine with Ktor DI (`provideGraphQl()`), off by default
...
is in [`docs/graphql.md`](../../../docs/graphql.md).
