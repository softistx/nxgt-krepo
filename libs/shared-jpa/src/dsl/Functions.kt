 ...
 *
 * ```kotlin
 * where { lower(this[Buyer::name]) eq term.lowercase() }
 * ```
 *
 * They are ordinary functions rather than scope members, so they nest and compose the way they read
 * — `length(trim(this[Buyer::name]))` — and a caller who needs one that is not here writes it the
 * same way, or reaches for [function] and names it.
 */
