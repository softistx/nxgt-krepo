 ...
 * one — so `eq`, `gt`, `oneOf`, `like` and the rest read the same on a hand-built criteria as they
 * do inside `select { }`.
 */
 ...
 *
 * Unlike [Joins.join] inside a query scope, this one is not remembered: two calls are two joins, as
 * they are in Criteria itself. Hold it in a `val`, which is the shape a hand-built criteria has
 * anyway.
 */
