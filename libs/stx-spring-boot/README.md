...
a request-values processor, both of which an interface generated by `plugins/openapi` needs. A
factory built per interface rebuilds the client underneath it, and the second interface is where one
of those settings quietly goes missing. This is also how a spec drives a controller through the
interface it implements; the `openapi-spec-first` skill has that convention.
