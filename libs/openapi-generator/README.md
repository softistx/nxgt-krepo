...
```
com.strange.openapi            the shared contract: the IR, SourceEmitter, EmitOptions
com.strange.openapi.parser     reading a document — OpenApiParser and everything it needs
com.strange.openapi.emit       building blocks every emitter uses: type mapping, interface files
...
The root package holds only what every other package depends on. Nothing there knows how a document
is read or what a client looks like, which is what keeps `parser` and the emitters from acquiring
...
3. Add a value to `ClientKind` in `plugins/openapi/src/settings.kt` and a branch to
   `ClientKind.emitter()` — those two lines are the only edit outside your own package.
4. Add a test under `test/<package>/` driving `SAMPLE_MODEL` from `test/Fixtures.kt`, so the new
...
parser change that breaks on a real document fails here rather than in a consuming module.
