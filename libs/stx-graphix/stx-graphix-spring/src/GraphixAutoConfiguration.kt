 ...
 *
 * **Every operation carries its [ServerWebExchange]**, over POST, over SSE and over graphql-ws
 * alike, so a controller method takes one as a parameter and a [GraphixInterceptor] reads it as
 * [GraphixChain.exchange].
 */
@AutoConfiguration
...
class GraphixAutoConfiguration {
     ...
     *
     * What shapes the schema arrives as an [ObjectProvider] rather than a `getBeansOfType` lookup,
     * so `orderedStream()` honours `@Order` — which is the difference between an interceptor that
     * authenticates before one that audits and one that does not.
     */
    @Bean
    ...
    fun graphix(
        ...
        scalars: ObjectProvider<GraphQLScalarType>,
        directives: ObjectProvider<GraphixDirective>,
        ...
        return Graphix {
            ...
            // The exchange is the framework parameter this module owns: every route puts one in the
            // operation context, so registering it here is what makes a controller method taking a
            // ServerWebExchange build instead of being told it needs @Argument.
            contextParameter(ServerWebExchange::class)
            resolvers(controllers)
            scalars.orderedStream().forEach { scalar(it) }
            directives.orderedStream().forEach { fieldDirective(it) }
