    ...
    FeatureSpec({
        ...
        feature("a palette grown from a seed") {
            scenario("is the same palette every time for the same seed") {
                val first = stxColors(seed, isDark = false)
                val second = stxColors(seed, isDark = false)
            ...
            scenario("differs between light and dark") {
                val light = stxColors(seed, isDark = false)
                val dark = stxColors(seed, isDark = true)
            ...
            scenario("answers a different seed with a different palette") {
                val other = stxColors(Color(0xFFDD4444), isDark = false)

                stxColors(seed, isDark = false).scheme.primary shouldNotBe other.scheme.primary
            }
        }

        feature("the three roles Material 3 does not have") {
            scenario("gives every tone a distinct main colour") {
                val colors = stxColors(seed, isDark = false)
                val mains =
            ...
            scenario("flips the semantic roles with the theme, not just the M3 ones") {
                val light = stxColors(seed, isDark = false).tone(Tone.Warning)
                val dark = stxColors(seed, isDark = true).tone(Tone.Warning)
        ...
        feature("adopting a scheme a caller already has") {
            scenario("keeps that scheme untouched and only adds the semantic roles") {
                val mine = stxColors(Color(0xFF008080), isDark = false).scheme

                stxColors(mine, isDark = false).scheme shouldBe mine
            }
        }
    })
