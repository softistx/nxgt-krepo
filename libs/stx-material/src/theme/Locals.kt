 ...
 * previews it outside a [StxTheme] — a missing theme should look plain, not crash.
 */
val LocalStrangeSpacing = staticCompositionLocalOf { StxSpacing() }

val LocalStrangeMotion = staticCompositionLocalOf { StxMotion() }

val LocalStrangeColors =
    staticCompositionLocalOf { stxColors(seed = DefaultSeed, isDark = false) }

/**
 * Material 3's own [Shapes], mirrored so a `Style` can read them.
 *
 * This is a mirror, not a second ladder: it holds the very instance [StxTheme] hands
