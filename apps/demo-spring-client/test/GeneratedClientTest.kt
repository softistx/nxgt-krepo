 ...
 */
class GeneratedClientTest : FunSpec({

    test("the generated interface is annotated for Spring") {
        CategoriesApi::class.java.getAnnotation(HttpExchange::class.java).shouldNotBeNull()
    }

    test("an operation carries its verb, url and path bindings") {
        ...
        method.parameterAnnotations.flatMap { it.toList() }
            .filterIsInstance<PathVariable>().single().name shouldBe "id"
    }

    test("an operation with a body declares its content type") {
        ...
        method.parameterAnnotations.flatMap { it.toList() }
            .filterIsInstance<RequestBody>().shouldNotBeEmpty()
    }

    test("optional query parameters opt out of Spring's required-value check") {
        val method = CategoriesApi::class.java.methods.single { it.name == "findCategories" }

        val cursor = method.parameterAnnotations.flatMap { it.toList() }
            .filterIsInstance<RequestParam>().single { it.name == "cursor" }
